...
frappe.ui.form.on('Employee Appointment', {
	setup: function(frm) {
		frm.custom_make_buttons = {
			'Vital Signs': 'Vital Signs',
			'Customer Encounter': 'Customer Encounter'
		};
	},
	...
	get_employee(frm){
		frappe.call({
			method: "lsc_api.lsc_api.doctype.employee_appointment.employee_appointment.get_employee_with_least_appointments",  // Update with the correct path to your Python function
			callback: function(r) {
				console.log(r)
				if (r.message) {
					frm.set_value('employee', r.message);
					frm.refresh_field('employee');
					frappe.show_alert({message: __('Employee has been set to ' + r.message), indicator: 'green'});
				} else {
					frappe.msgprint(__('No employee found.'));
				}
			}
		});
	},
	refresh: function(frm) {
		...
		if (["Open", "Checked In"].includes(frm.doc.status) || (frm.doc.status == 'Scheduled' && !frm.doc.__islocal)) {
			...
			if (frm.doc.procedure_template) {
				...
				frm.add_custom_button(__('Customer Encounter'), function() {
					frappe.model.open_mapped_doc({
	...
	appointment_for: function(frm) {
		if (frm.doc.appointment_for == 'Employee') {
			if (!frm.doc.employee) {
				...
				callback: function(r) {
					if (r.message) {
						...
						frappe.msgprint({
							...
							message: __('There are no Therapies prescribed for Customer {0}', [frm.doc.customer.bold()]),
							indicator: 'blue'
						});
					}
				}
...
let check_and_set_availability = function(frm) {
	...
	function show_availability() {
		...
		let d = new frappe.ui.Dialog({
			...
			fields: [
				{ fieldtype: 'Link', options: 'Department', reqd: 1, fieldname: 'department', label: 'Department' },
		...
		d.fields_dict['employee'].df.onchange = () => {
			if (d.get_value('employee') && d.get_value('employee') != selected_practitioner) {
				selected_practitioner = d.get_value('employee');
				show_slots(d, fd);
			}
		};
	...
	function show_slots(d, fd) {
		if (d.get_value('appointment_date') && d.get_value('employee')) {
			...
			frappe.call({
				method: 'lsc_api.lsc_api.doctype.employee_appointment.employee_appointment.get_availability_data',
				args: {
	...
	function get_slots(slot_details, fee_validity, appointment_date) {
		...
		slot_details.forEach((slot_info) => {
			...
			slot_html += `
				...
				<span><b> ${__('Employee Name: ')} </b> ${slot_info.service_unit}</span>`;
		...
		frappe.call({
			...
			callback: function(r) {
				if (r.message && r.message.length) {
					...
					frappe.msgprint({
						title: __('Not Found'),
						message: __('No Prescribed Procedures found for the selected Customer')
					});
				}
			}
		});
...
let show_procedure_templates = function(frm, result) {
	...
	$.each(result, function(x, y) {
		...
		<div class="col-xs-5"> %(procedure_template)s <br>%(employee)s  <br> %(date)s</div>\
		<div class="col-xs-2">\
		<a data-name="%(name)s" data-procedure-template="%(procedure_template)s"\
		data-encounter="%(encounter)s" data-employee="%(employee)s"\
		...
		<button class="btn btn-default btn-xs">Add\
			...
			employee: y[5] ? y[5] : '', date: y[6] ? y[6] : '', department: y[7] ? y[7] : ''
		})).appendTo(html_field);
		row.find("a").click(function() {
			frm.doc.procedure_template = $(this).attr('data-procedure-template');
			frm.doc.procedure_prescription = $(this).attr('data-name');
			frm.doc.employee = $(this).attr('data-employee');
	...
	if (!result) {
		let msg = __('There are no procedure prescribed for ') + frm.doc.customer;
...
let show_therapy_types = function(frm, result) {
	...
	$.each(result, function(x, y) {
		...
		<div class="col-xs-5"> %(encounter)s <br> %(employee)s <br> %(date)s </div>\
		...
		<button class="btn btn-default btn-xs">Add\
			...
			therapy: y[0],
			name: y[1], encounter: y[2], employee: y[3], date: y[4],
...
let update_status = function(frm, status) {
	...
	frappe.confirm(__('Are you sure you want to cancel this appointment?'),
		function() {
			frappe.call({
				method: 'lsc_api.lsc_api.doctype.employee_appointment.employee_appointment.update_status',
...
let make_payment = function (frm, automate_invoicing) {
	...
	function show_payment_dialog(frm, fields) {
		...
		if (frm.doc.appointment_for == "Employee") {
			d.set_value("employee", frm.doc.employee_name);
		} else if (frm.doc.appointment_for == "Service Unit") {
